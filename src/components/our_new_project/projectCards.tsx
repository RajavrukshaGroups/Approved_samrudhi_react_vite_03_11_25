...
// export default function ProjectCard({ title, image, isSecondProject = false,logo }: ProjectCardProps) {
export default function ProjectCard({
  title,
  image,
  logo,
  isSecondProject = false,
}: ProjectCardProps) { 
return (
    ...
    // </div>
      ...
      <div
        className={`relative flex-1 rounded-xl overflow-hidden mb-6 ${
          isSecondProject ? "overflow-hidden" : ""
        }`}
      >
        {/* Background Image */}
        <img
        ...
        {/* Company Logo positioned above the image */}
        <div className="absolute top-24 left-1/2 -translate-x-1/2 z-10">
          <img
            src={logo}
            alt="Company Logo"
            className="w-28 h-auto transition-transform duration-500 group-hover:scale-110"
          />
        </div>

        {/* Overlay effect (optional for better contrast) */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent"></div>
      </div>

      {/* View Project Details Link */}
      <div className="flex items-center justify-between border-gray-200 p-2">
        <span className="text-yellow-500 font-medium">View Project Details</span>
        <svg
          className="w-5 h-5 text-green-600 transition-transform duration-300 group-hover:translate-x-1"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
